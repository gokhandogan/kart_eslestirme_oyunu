...
public class Config
{
    ...
    public static void PlaceScoreOnBoard(float time)
    {
        ...
        switch (GameSettings.Instance.GetEPairNumber())
        {
    ...
    public static void PlaceScoreOnBoard(float time, float[] scoreTimeList, string[] pairNumberList)
    {
        ...
        for (int i = 0; i < NumberOfScoreRecords; i++)
        {
            if (scoreTimeList[i]>time || scoreTimeList[i] == 0.0f)
            {
                ...
                for (var moveDownForm = (NumberOfScoreRecords-1); moveDownForm > i; moveDownForm--)
                {
                    scoreTimeList[moveDownForm] = scoreTimeList[moveDownForm - 1];
                    pairNumberList[moveDownForm] = pairNumberList[moveDownForm - 1];
                }
